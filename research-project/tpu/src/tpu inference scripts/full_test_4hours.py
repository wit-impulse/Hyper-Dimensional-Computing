...
def test_results(model_name,mode,samples,accuracy,test_time,tb,te, device):
    if os.path.isfile("results/accuracy/test.csv"):
          #print("File exist writing data to existing file!")
          data=[model_name,mode,samples,accuracy,test_time, tb, te, device]
      ...
      print ("Creating result file...")
      header = ['model','mode','samples','test accuracy','testing time','testing begin time','testing end time', 'device']  
      data=[model_name,mode,samples,accuracy,test_time,tb, te, device]
...
while t()-main_time<14400:
    tot_infer_time=0
    pred=[]
    accuracy=0
    
    test_begin_time=t()-main_time
    for image in images:
    ...
    test_end_time=t()-main_time
    test_results(model_name,mode,samples,accuracy,tot_infer_time,test_begin_time, test_end_time, device)
# accuracy = (sum(1 for x,y in zip(test_labels,pred) if x == y  )* 100) / len(test_images)
with flag.get_lock():
    flag.value = True
p1.join()
